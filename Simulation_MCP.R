########################################################
#
# This file provides how to obtain simulation results -- asymmetric mispricing correction persistence mechanism
#
########################################################
...
out<-matrix(nrow = 100, ncol = 10)
num <- 1

for (sigma_epsilon in c(0.07,0.1))
{ 
for (phi_l in c(0.15,0.99))  
{ 
  ...
  if (phi_l == 0.99)
  {
    out[num+1,1] <- 'Strong asymmetric MCP'
  }
  else
  {
    out[num+1,1] <- 'Weak asymmetric MCP'
  }
  
  out[num:(num+5),2:10] <- as.matrix(cbind(longshort,long,short))
...
write.xlsx(data.frame(out),file = "./Table/simulation_MCP.xlsx",colnames = TRUE)
