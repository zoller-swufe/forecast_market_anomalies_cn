########################################################
#
# This file provides how to obtain forecast results
#
########################################################
...
# Storage list object for ENet coefficient estimates

coefEnetAll = rep(list(matrix(NA, p, length(nameAnomaly))),
                   times = length(h))

names(coefEnetAll) = names(h)
...
for ( s in 1:p ){
  ...
  for ( i in 1:length(h) ){
    ...
    if ( s > pHold ){
      ...
      # Store Enet slope estimates
      coefEnetAll [[ i ]][ s , ] = siResult$b[ -1 ]
...
# Storage list object for ENet coefficient estimates
coefEnet = rep(list(matrix(NA, p, 132)),
                  times = 2)

names(coefEnet) = c(1,2)
...
for ( i in 1:length(leg) ){
  ...
  # Clear up some memory
    rm.all.but(keep = c('leg', 'nameLeg', 'r', 'pHold', 'coefEnet'),
             keep_functions = TRUE)
